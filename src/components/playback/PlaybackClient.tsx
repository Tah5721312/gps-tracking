...
export default function PlaybackClient() {
  ...
  useEffect(() => {
    ...
    // التحقق من أن الخريطة غير مهيأة بالفعل
    if (isMapInitialized.current || mapRef.current) return;

    // التحقق من أن الـ container موجود
    if (!mapContainerRef.current) return;

    // التحقق من أن الـ container ليس لديه خريطة مسبقاً
    ...
    const init = async () => {
      try {
        // التحقق مرة أخرى قبل التهيئة
        if (isMapInitialized.current || mapRef.current || !mapContainerRef.current) return;
        if ((mapContainerRef.current as any)._leaflet_id) {
          isMapInitialized.current = true;
          return;
        }
    ...
    <div className="min-h-screen bg-linear-to-br from-blue-50 via-white to-purple-50" dir="rtl">
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        ...
        <div className="flex flex-col gap-6 mt-6">
          <div className="bg-white/90 backdrop-blur-sm rounded-2xl shadow-xl p-6 border border-gray-200/50 hover:shadow-2xl transition-all duration-300 order-1">
          ...
          <div className="bg-white/90 backdrop-blur-sm rounded-2xl shadow-xl overflow-hidden border border-gray-200/50 hover:shadow-2xl transition-shadow duration-300 order-2">
            <div
              ref={mapContainerRef}
              className="w-full"
              style={{
